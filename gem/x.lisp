...
(defun do-client-message (event-window type data format display)
  (cond ((and (eq format 32)
	...
	((and (eq format 32)
	 ...
	 (if (trans-from-file *event-receiver*)
	   T
	   ;; ignore events when read transcript
	   ;; (interactors::Queue-Timer-Event (aref (the (simple-array
	   ;; 					       (unsigned-byte 32) (5))
	   ;; 					      data) 0))
	   (Queue-Timer-Event *event-receiver* (aref (the (simple-array
						     (unsigned-byte 32) (5))
						    data) 0))
	   )))
  NIL)
  ...
  (let ((display (the-display root-window))
    ...
    (xlib:event-case
     ...
     (:MAP-NOTIFY
      ...
      (do-map-notify receiver (x-window-from-drawable root-window event-window)))
     (:UNMAP-NOTIFY
      (event-window)
      (event-handler-debug :UNMAP-NOTIFY)
      (do-unmap-notify receiver
	...
	(let ((window (x-window-from-drawable root-window event-window)))
     ...
     (:CIRCULATE-NOTIFY
      ...
      (do-circulate-notify receiver))
     (:GRAVITY-NOTIFY
      ()
      (event-handler-debug :GRAVITY-NOTIFY)
      (do-gravity-notify receiver))
     (:DESTROY-NOTIFY
      (event-window)
     ...
     (:CONFIGURE-NOTIFY
      ...
      (if (connected-window-p event-window)
	(do-configure-notify receiver (x-window-from-drawable root-window
							      event-window)
     ...
     (:EXPOSURE
      ...
      (if (connected-window-p event-window)
	(do-exposure receiver (x-window-from-drawable root-window event-window)
	...
	(do-button-release receiver
     ...
     (:MOTION-NOTIFY
      ...
      (unless ignore-keys
	(do-motion-notify receiver
	  (x-window-from-drawable root-window event-window) x y display)))
     (:ENTER-NOTIFY
      (event-window x y time)
      (event-handler-debug :ENTER-NOTIFY event-window x y time)
      (unless ignore-keys
	(do-enter-notify receiver (x-window-from-drawable root-window event-window)
	  x y time)))
     (:LEAVE-NOTIFY
      (event-window x y time)
      (event-handler-debug :LEAVE-NOTIFY event-window x y time)
      (unless ignore-keys
	(do-leave-notify receiver
	  (x-window-from-drawable root-window event-window) x y time)))
     (:NO-EXPOSURE
      ()
