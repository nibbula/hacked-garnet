...
#-(and cmu (not mp)) (defvar *main-event-loop-process-was-halted* nil)
...
(defun Disconnect-Garnet ()
  ...
  #+(and cmu (not mp))
  (ext:disable-clx-event-handling opal::*default-x-display*)
...
;    (setf (aref *colormap-index-table* n) 0))
  ;;we can do this simpler thing because we use a default value of zero
  ;;for the table entries [1995/09/12:goldman]
  (setf *colormap-index-table* (make-hash-table :size 256))
...
(defun Reconnect-Garnet (&optional display-name device-type)
  ...
  (let ((current-device (g-value DEVICE-INFO :current-device))
    ...
    (let ( #+allegro (excl:*redefinition-warnings* NIL)
           #+apple (ccl:*warn-if-redefine* NIL))
    ...
    #-apple
    (opal::set-draw-functions)		; defined in defs.lisp

    ;;the following block of code was erroroneous, because you can't call
    ;;alloc-color-cells on a :true-color or :static-color screen...
    ;;conditionalized it. [1995/12/08:goldman]
