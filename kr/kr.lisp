	...
	(let ((entry (slot-accessor parent slot))
	  ...
	  (unless (eq value *no-value*)
		...
		(set-cache-is-valid value NIL))
	      ...
	      (if is-leaf
		(if slot-structure	; slot had constant bit
  ...
  (let ((inverse (cadr (assocq slot *relations*))))
	...
	(if entry
	  (dolist (parent (sl-value entry))
	    (if (not-deleted-p parent)	; parent is not destroyed
...
(defun link-in-relation (schema slot values)
  (let ((inverse (if (eq slot :is-a)
	...
	(let* ((entry (if (eq slot :is-a)
	       ...
	       (previous-values (if entry (sl-value entry))))
	  (if entry
    ...
    (cond ((consp complex-type);; complex type (a list)
	   (let ((fn   (first complex-type))
		...
		(let* ((pred (find-lisp-predicate fn))
		       (lo (first args))
		       (lo-expr (if (and lo (not (eq lo '*)))
				  (if (listp lo)
				    `((< ,(car lo) value))
				    `((<= ,lo value)))))
		       (hi (second args))
		       (hi-expr (if (and hi (not (eq hi '*)))
				  (if (listp hi)
				    `((> ,(car hi) value))
				    `((>= ,hi value))))))
   ...
   (when (>= *next-type-code* curlen)
    ...
    (let ((newlen (+ curlen *types-array-inc*)))
     (setf types-array     (copy-extend-array types-array     curlen newlen)
           ...
           type-docs-array (copy-extend-array type-docs-array curlen newlen))))   ;; in any case, return current code, then add one to it
   (prog1
     *next-type-code*
     (incf *next-type-code*))))
...
(defun recompute-formula (schema slot)
  (let* ((entry (slot-accessor schema slot))
	 (formula (if entry (sl-value entry))))      
    (if (formula-p formula)
...
(defun visit-inherited-values (schema a-slot function)
  (let* ((entry (slot-accessor schema a-slot))
	 (parent-entry (if entry (sl-value entry))))
    (dolist (inverse *inheritance-inverse-relations*)
...
;;;
(defmacro check-not-constant (schema slot entry)
...
(defun s-value-fn (schema slot value)
  ...
  (let* ((entry (slot-accessor schema slot))
	...
	(run-pre-set-demons schema slot value NIL :S-VALUE))
	 ...
	 (if (zerop (a-formula-number old-value))
 ...
 re-install the formula using s-value.~%"
	...
	(t
	 ;; All other cases
	 (if (and is-formula (null (cached-value value)))
	   ;; place old value in the cache only if an initial value
	   ;; was not provided for the new formula
	   ;; Set value, but keep formula invalid.
...
(defun reset-inherited-values (schema)
  (iterate-slot-value (schema T NIL T)	; use inheritance!
    (unless (relation-p slot)
      (unless (eq value *no-value*)
	(if (is-inherited (sl-bits iterate-slot-value-entry))
...
;;;
(defun call-on-ps-slots (schema function &key (control t) inherit (indent NIL)
				types-p all-p)  
	  ...
	  ((and values (listp values) (listp (cdr values)))
	   ...
	   (dolist (value values)
	     ...
	     (when (and limit-values (> number limit-values))
	       ...
	       (return nil)))
	   #+clisp
	   ;; This clause required for printing dotted pairs in CLISP
	   (loop for value = values then (cdr value) while value
                 do (setf printed t)
			     ; allow dotted lists - Bill Wallace
                 do (when (not (consp value))
                      (format t " . ~S" value)
                      (return nil))
                 do (setf column (print-one-slot-helper (car value)
                                                        column indent
                                                        (> number 0) nil))
                 do (incf number)
                 do (when (and limit-values (> number limit-values))
	              ;; Too many values: use ellipsis form.
                      (format t " ...")
                      (return nil)))
	   (format t ")")
...
;;; garbage heap, which is kept in the <array>.  The resource must have
;;; at least <how-many> entries.
...
(defun find-unused-resource (array how-many)
  (dotimes (i (length array))
	...
	(if (/= current-length how-many)
	  (adjust-array slots how-many))
...
(defun merge-prototype-values (object slot parents values)
  (unless values
    (setf values (if parents (g-value (car parents) slot))))
  (let ((exceptions nil)
	(add-prototype nil)
	(results nil))
...
(defun process-constant-slots (the-schema parents constants do-types)
 ...
 the expression ~S instead."
	...
	(return))
      ...
      (let ((slot (car local)))
	(if (null (slot-accessor the-schema slot))
	  (if (second local)
	...
	(let ((bits (sl-bits iterate-slot-value-entry))) ; get parent's bits
	  ...
	  (unless (zerop bits)
		...
		(let ((schema-bits (sl-bits the-entry)))
		  (if (zerop (extract-type-code schema-bits))
...
;;; an argument list suitable for do-schema-body.  It is called at compile
;;; time.
...
(defun do-schema-body (schema is-a generate-instance do-constants override
  ...
  (do* ((slots slot-specifiers (cdr slots))
	...
	(unless (eq types :NONE)
	  (dolist (type types)
		...
		(dolist (slot (cdr type))
	      ...
	      (format t "*** ERROR - empty list of slots in type declaration for object ~S:~%  ~S~%" schema (car type)))))
	...
	(dolist (slot slot-specifiers)
	  (if (and (listp slot)
		   (memberq (car slot) '(:IGNORED-SLOTS :LOCAL-ONLY-SLOTS
					:MAYBE-CONSTANT :PARAMETERS :OUTPUT
					:SORTED-SLOTS :UPDATE-SLOTS)))
	  ...
	  ((consp slot)
	   (let ((slot-name (car slot))
		...
		(setf had-constants T)))
	     ...
	     (if (and (not cancel-constants) (not *constants-disabled*)
		      (not *redefine-ok*))
	       (if (and parent (slot-constant-p parent slot-name))
		 (cerror
...
(defmacro get-slot-type-code (object slot)
  (let ((entry (gensym)))
...
(let ((realp-symbol (find-symbol "REALP" 'lisp)))
  (unless (and realp-symbol (fboundp realp-symbol))
    (setf (symbol-function (intern "REALP" 'lisp))
	  (symbol-function 'numberp))))
