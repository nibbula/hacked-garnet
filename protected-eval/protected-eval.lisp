...
15-Nov-2002 FMG Moved the little remaining stuff from
                protected-eval.lisp that wasn't already here
                into this file.  Now protected-eval.lisp can
                go away. But that means this file could be
                called protected-eval.lisp instead of
                new-protected-eval.lisp.  Made it so.

15-Nov-2002 FMG Fixed problems with with-garnet-error-handling and
                with-garnet-user-error-handling macros.

8/27/93 RGA Wrapped With-hourglass-Cursor around error handler
            gadgets.  

8/18/93  RGA Fixed bugs related to new version of Garnet and added
	     documentation support.

01/25/93  RGA  Created  modifying from old-protected-eval. so that we can
               add handling for USE-VALUE and STORE-VALUE restarts.
...
(kr:create-instance 'PROTECTED-EVAL-ERROR-GADGET GG::Motif-Query-Gadget
  #+kr-doc
  (:documentation
   "Error Gadget Used by Garnet-Error-Handler to Display error messages." )
  (:parent-window nil)
  (:modal-p t)
  (:beep-p t)
...
general be system dependent and need to be hacked."
  (if (find-restart 'abort)
    (abort)	;Worked!
    (throw 'cl::top-level-catcher t)))


#-(and cmu allegro)
(defun do-abort ()
  ;;this will generate error which means you need to find a solution
  ;;for your lisp.
  (abort))
